...
const MediaCoverage = () => {
  ...
  const filteredItems = activeCategory === "all" ? newsMentions : newsMentions.filter(item => item.category === activeCategory);
  ...
  return <section className="py-16 bg-white" id="media-coverage" ref={inViewRef}>
      <div className="container mx-auto px-4">
        <div className={`text-center mb-12 transition-opacity duration-700 ${isInView ? 'opacity-100' : 'opacity-0'}`}>
        ...
        {/* Featured Carousel */}
        <div className={`relative mb-16 transition-all duration-1000 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          ...
          <div ref={carouselRef} className="flex overflow-x-auto pb-4 space-x-4 scrollbar-hide" style={{
          scrollbarWidth: 'none',
          msOverflowStyle: 'none'
        }}>
            {featuredItems.map(item => <div key={item.id} className="flex-none w-[300px] md:w-[350px]">
                <Card className="h-full overflow-hidden shadow-md hover:shadow-lg transition-shadow">
                  ...
                  <CardFooter className="p-3 bg-gray-50 text-sm text-gray-500 flex justify-between items-center">
                    ...
                    {item.link && <a href={item.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-medical-blue hover:text-medical-teal transition-colors">
        ...
        <div className={`bg-medical-blue text-white rounded-lg overflow-hidden shadow-xl mb-16 transition-all duration-1000 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <div className="p-8">
            <h3 className="text-2xl font-bold mb-6 text-center text-gray-50">Featured Video</h3>
            <div className="flex flex-col md:flex-row gap-8 items-center">
              <div className="md:w-1/2">
                <div className="rounded-lg overflow-hidden">
                  <img src="/lovable-uploads/80670cba-b70e-4db3-be7f-0cef3def222a.png" alt="Dr. Moosa Kunhi performing heart surgery" className="w-full h-auto" />
                </div>
              </div>
              <div className="md:w-1/2">
                ...
                <a href="https://youtu.be/XSimd7zRRhU?si=QOJD0MrnHt5Etdpw" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-white text-medical-blue px-4 py-2 rounded hover:bg-gray-100 transition-colors">
                  <Video size={18} />
                  Watch Video Feature
                </a>
              </div>
        ...
        <div className={`transition-all duration-1000 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            ...
            <div className="relative">
              <Tabs defaultValue="all" className="w-full">
                <TabsList className="grid grid-flow-col auto-cols-max gap-2 overflow-x-auto p-1 max-w-full">
                  {allCategories.map(category => <TabsTrigger key={category} value={category} onClick={() => setActiveCategory(category)} className="capitalize">
                      {category}
                    </TabsTrigger>)}
                </TabsList>
              </Tabs>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredItems.map(item => <Card key={item.id} className="overflow-hidden shadow-sm hover:shadow-md transition-shadow border-l-4 border-medical-blue">
                <CardHeader className="p-4 pb-2">
                  <div className="flex items-center gap-3 mb-2">
                    <span className={`p-2 rounded-full text-white ${item.type === 'video' ? 'bg-red-500' : item.type === 'award' ? 'bg-medical-gold' : item.type === 'social' ? 'bg-blue-500' : 'bg-medical-blue'}`}>
                      {getItemIcon(item.type)}
                    </span>
                ...
                <CardFooter className="p-3 bg-gray-50 flex justify-end">
                      ...
                      <DialogContent className="max-w-4xl">
                        ...
                        <div className="mt-4">
                          ...
                          <div className="flex justify-end">
                            <a href={item.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 bg-medical-blue text-white px-4 py-2 rounded-sm hover:bg-medical-blue-dark transition-colors">
                  ...
                  {item.link && <a href={item.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-medical-blue hover:text-medical-teal transition-colors text-sm ml-2">
        ...
        <div className={`bg-medical-gray p-6 rounded-lg shadow-sm mt-12 transition-all duration-1000 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
